...
// TestClaudeCodeDisableSlashCommands verifies --disable-slash-commands flag is passed when enabled.
func TestClaudeCodeDisableSlashCommands(t *testing.T) {
	d := testDB(t)
	s := New(d, 9001)
	...
	tests := []struct {
		...
		{name: "disabled by default", disableSlashCommands: false, wantFlag: false},
		{name: "enabled", disableSlashCommands: true, wantFlag: true},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			agent := &models.Agent{
				...
				Runner: "claude_code", Model: "sonnet",
				WorkingDir: t.TempDir(), MaxTurns: 10, TimeoutSec: 10,
				DisableSlashCommands: tt.disableSlashCommands,
			}
			out, _ := s.execClaudeCode(t.Context(), agent, "key", "run1", "SO-1", "task")
